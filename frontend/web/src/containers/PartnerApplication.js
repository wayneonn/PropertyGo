...
const PartnerApplication = () => {
  ...
  const [
    showAcceptPartnerApplicationModal,
    setShowAcceptPartnerApplicationModal,
  ] = useState(false);
  const [partnerApplicationId, setPartnerApplicationId] = useState(0);

  const itemsPerPage = 9;

  const [currentPagePartnerApplication, setCurrentPagePartnerApplication] =
    useState(1);
  const [totalPagePartnerApplication, setTotalPagePartnerApplication] =
    useState(0);

  const indexOfLastItemPartnerApplication =
    currentPagePartnerApplication * itemsPerPage;
  const indexOfFirstItemPartnerApplication =
    indexOfLastItemPartnerApplication - itemsPerPage;
  ...
  useEffect(() => {
    ...
    fetchApplicationDataFromServer().then((r) =>
      console.log("Fetch data from server activated.")
    );
  }, []);
  ...
  useEffect(() => {
    ...
    setTotalPagePartnerApplication(
  ...
  const handleApprove = async () => {
    try {
      await axios.put(
        `http://127.0.0.1:3000/user/partner/admin/approve/${partnerApplicationId}`
      );
      setShowAcceptPartnerApplicationModal(false);
      fetchApplicationDataFromServer();
  ...
  const handleReject = (application) => {
    ...
    setRejectionNote(
      application.adminNotes !== null ? application.adminNotes : ""
    );
  };
  ...
  const handleRejectNoteSubmission = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `http://127.0.0.1:3000/user/partner/admin/reject/${rejectId}`,
        {
      ...
      fetchApplicationDataFromServer().then(
        (r) => "Updated rejection successfully."
      );
      setShowRejectModal(false);
      setValidationMessages({});
      showToast("rejected");
  ...
  const fetchApplicationDataFromServer = async () => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:3000/user/partner/admin/approval"
      );
      console.log("Response is: ", response.data.partnerApp);

      const partnerApp = response.data.partnerApp.filter(
        (pa) => pa.adminNotes === null
      );
            ...
            <div>
                ...
                <Table hover responsive>
                  ...
                  <tbody>
                    {applications.length > 0 ? (
                      applications
                        .slice(
                          indexOfFirstItemPartnerApplication,
                          indexOfLastItemPartnerApplication
                        )
                        .map((application) => (
                          <tr
                            key={application.partnerApplicationId}
                            style={{ textAlign: "center" }}
                          >
                            <td>{application.companyName}</td>
                            <td>
                              {formatUserCreatedAt(application.createdAt)}
                            </td>
                            ...
                            <td>
                              <Button
                                variant="warning"
                                onClick={() =>
                                  toggleAcceptPartnerApplicationModal(
              ...
              <div>
                <Pagination className="faq-paginate">
                  {Array.from({ length: totalPagePartnerApplication }).map(
                    (_, index) => (
                      <Pagination.Item
                        key={index}
                        active={index + 1 === currentPagePartnerApplication}
                        onClick={() =>
                          handlePageChangePartnerApplication(index + 1)
                        }
                      >
                        {index + 1}
                      </Pagination.Item>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: "20px" }}>
            Approval of Partner Application
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <tbody>
              {documentDetails.map((doc) => (
                <tr key={doc.documentId} style={{ textAlign: "center" }}>
                  ...
                  <td>
                    <Button onClick={() => handleDownload(doc.documentId)}>
                      {" "}
                      View{" "}
                    </Button>
                  </td>
          ...
          <Form>
            <Form.Group controlId="rejectionNote">
              ...
              <Form.Control
                ...
                isInvalid={validationMessages.emptyRejectionMessage}
              />
