...
import { updateForumCommentFlaggedStatus, deleteForumComment, getAllUserFlaggedForumComment} from '../../utils/forumCommentApi';
import { getAllForumPostById } from '../../utils/forumPostApi';
import ForumCommentItem from '../../components/Forum/ForumCommenItem';


const FlaggedComments = ({ navigation }) => {
    ...
    const useParentCallback = useCallback(() => {
        const fetchData = async () => {
            try {
                const forumComment = await getAllUserFlaggedForumComment(user.user.userId, false);
