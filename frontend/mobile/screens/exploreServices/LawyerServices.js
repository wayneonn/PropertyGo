...
const LawyerServices = ({navigation}) => {
    ...
    const useParentCallback = useCallback(() => {
        ...
        fetchLawyers().then(r => console.log("Fetch lawyer."));
    }, [])
    ...
    const fetchLawyers = async() => {
        try {
            ...
            setFilteredLawyer(lawyers.partnerInfo.sort((a, b) => {
        ...
        <SafeAreaView style={styles.container}>
            <ScrollView refreshControl={
                ...
                <View style={[styles.scene, {backgroundColor: '#f3f3f3'}]}>
                    {filteredLawyer.length !== 0 ? filteredLawyer.map((item) => (
                        <TouchableOpacity
                ...
                <LawyerModal modalImage={modalImage} modalVisible={modalVisible} setModalVisible={setModalVisible}
                             handleChatWithLawyer={handleChatWithLawyer} selectedLawyer={selectedLawyer} navigation={navigation}/>
            </ScrollView>
        </SafeAreaView>
    )
}
