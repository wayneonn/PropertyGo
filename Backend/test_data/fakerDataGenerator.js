...
exports.generateFakeProperties = async(numOfRecords) => {
    ...
    for (let i = 0; i < numOfRecords; i++) {
        const fakeProperty = {
            ...
            offeredPrice: faker.datatype.boolean() ? parseFloat(faker.finance.amount(9000, 95000, 2)) : null,
            ...
            propertyStatus: faker.helpers.arrayElement(['ACTIVE', 'ON_HOLD', 'COMPLETED']),
            postalCode: faker.number.int({min: 1, max: 999999}),
            unitNumber: faker.number.int({min: 1, max: 1000}).toString(),
            address: faker.location.streetAddress(),
            area: faker.location.city(),
            region: faker.location.state(),
            boostListingStartDate: faker.datatype.boolean() ? faker.date.between({from: '2023-01-01', to:'2023-12-31'}) : null,
