...
async function uploadPartnerChatPictures(req, res) {
  // Start a transaction
  const transaction = await sequelize.transaction();
  try {
    const images = req.files;
    ...
    for (let index = 0; index < images.length; index++) {
      const image = images[index];
      console.log('Creating image for index', index, 'with Chat ID', req.params.id);
      ...
      try {
        ...
        const imageData = {
          // title: `Image ${index + 1}`,
          image: processedImageBuffer,
          chatId: req.params.id, // Associate the image with the created property
        };

        // Create the image record with the associated chatId
        let new_img = await Image.create(imageData, { transaction });
        // Push the image ID in.
        successImages.push(new_img.imageId)
        console.log(`Image ${index + 1} created successfully.`);
      } catch (imageError) {
        console.error('Error creating image:', imageError);
        failedImages.push({ index, error: 'Failed to create image' });
    ...
    // Inform image update.
    req.io.emit("newImageLoaded", "New image update has been called.");
    res.status(200).json({ message: 'Photos saved successfully', successImages});
  } catch (error) {
    console.error("Error uploading company photos: ", error)
  }
