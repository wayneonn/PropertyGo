...
const markAllAsRead = async (req, res) => {
  ...
  for (const notification of notifications) {
    ...
    const foundNotification = await Notification.findByPk(notification.notificationId);
    await foundNotification.update(req.body);
  }

  return res.status(200).json({ message: "All notifcations have been marked as read"});
};
